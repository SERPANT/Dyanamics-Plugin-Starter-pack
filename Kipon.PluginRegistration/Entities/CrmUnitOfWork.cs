...
namespace OrganizationName.PluginRegistration.Entities
{
    public class CrmUnitOfWork : IUnitOfWork, IDisposable
    {
        ...
        {
            var section = OrganizationName.PluginRegistration.Security.Configuration.ConfigurationSingleton.Section;

            var serviceUri = new Uri(section.XrmServiceUrl);
            var orgServiceManagement = ServiceConfigurationFactory.CreateManagement<IOrganizationService>(serviceUri);

            var credentials = OrganizationName.PluginRegistration.Security.Xrm.AuthenticationHelper.GetCredentials(orgServiceManagement, OrganizationName.PluginRegistration.Security.Configuration.ConfigurationSingleton.Section.XrmAuthenticationProviderType);
            var orgService = OrganizationName.PluginRegistration.Security.Xrm.AuthenticationHelper.GetProxy<IOrganizationService, OrganizationServiceProxy>(orgServiceManagement, credentials);
