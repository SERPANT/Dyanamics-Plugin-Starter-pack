...
namespace OrganizationName.Dynamics.Plugin.DI
{
    public class ServiceFactory
    {
        ...
        private static Dictionary<Type, Type> GetTypesWithExportAttribute()
        {
            ...
            var exportAttribute = typeof(OrganizationName.Dynamics.Plugin.DI.Export);
            var result = new Dictionary<Type, Type>();
            foreach (Type type in assembly.GetTypes())
            {
                var ea = type.GetCustomAttributes(exportAttribute, true).SingleOrDefault() as OrganizationName.Dynamics.Plugin.DI.Export;
                if (ea != null)
                {
                    result[ea.Type] = type;
                }
        ...
        internal void Initialize(Dictionary<Type, object> context, object obj)
        {
            ...
            {
                var imp = prop.GetCustomAttributes(typeof(OrganizationName.Dynamics.Plugin.DI.Import), true).SingleOrDefault();
                if (imp != null)
                {
